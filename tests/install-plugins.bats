...
@test "[${SUT_DESCRIPTION}] plugins are installed with install-plugins.sh but with a depreciation warning message" {
  run docker run --rm -e CURL_OPTIONS='--verbose --location --silent --show-error --fail' "${SUT_IMAGE}" bash -c 'install-plugins.sh junit:1.6 && ls --color=never -1 /usr/share/jenkins/ref/plugins/'
  assert_success
