...
: "${OPENJ9_JAVA_OPTIONS:="-Xshareclasses,readonly -Xscmx"}"
...
if [[ $# -lt 1 ]] || [[ "$1" == "--"* ]]; then
  ...
  exec java -Duser.home="$JENKINS_HOME" "${java_opts_array[@]}" ${OPENJ9_JAVA_OPTIONS} -jar ${JENKINS_WAR} "${jenkins_opts_array[@]}" "$@"
fi
